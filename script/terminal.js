	...
	let textReplies = function () {
		switch (textInputValueLowerCase) {
			...
			case "ls":
				...
				addTextToResults("Evolução");
				addTextToResults("Declinio");
				addTextToResults("Referências");
				addTextToResults("Organização");
			...
			case "cat organizacao":
				clearInput();
				addTextToResults("SISTEMA DE DIRETÓRIOS: Raíz do sistema Unix é /. Neste diretório se encontra os principais diretórios do sistema:");
				addTextToResults('Diretórios Importantes: boot É um arquivo que contém informações de inicialização do sistema.É o primeiro a ser executado após ligar a máquina. dev Contém arquivos que realizam a conexão do sistema com periféricos, como teclado e leitor de cassetes. etc Contém arquivos administrativos, como informações senhas e outros arquivos de sistema. lib Contém o compilador da linguagem C. tmp Contém arquivos temporários do sistema. unix É o programa onde se encontra o kernel do Unix, o primeiro a ser executado pela máquina após o boot. usr Contém os arquivos criados pelos usuários.');
				addTextToResults('SISTEMA DE ARQUIVOS: Os arquivos do Unix nos anos 80 não possuíam extensão. Ou seja, todos os arquivos são diferenciados somente pelo seu nome e diretório onde se encontra. Apesar de existirem convenções, o comando file, responsável por identificar o tipo de arquivo, era executado reconhecendo características específicas no arquivo em questão, como #include em um arquivo de programa escrito em C ou uma sequência de binários em um arquivo binário. O que definia a "extensão" do arquivo era o programa que o executava funcionalmente.');
				break;
			...
			case "tail organizacao":
				clearInput();
				addTextToResults('"extensão" do arquivo era o programa que o executava funcionalmente.');
				break;
